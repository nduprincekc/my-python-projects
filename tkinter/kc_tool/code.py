...
l2 = Label(LHS, text=f"Version:{my_system.version}", bg='#f4f5f5', font=('Acumin Variable Concept', 15),
           justify='center')
l2.place(x=20, y=225)
...
l4 = Label(LHS, text=f"Machine:{my_system.machine}", bg='#f4f5f5', font=('Acumin Variable Concept', 15),
           justify='center')
l4.place(x=20, y=275)

l5 = Label(LHS, text=f"Total Ram Installed:{round(psutil.virtual_memory().total / 1000000000)}GB", bg='#f4f5f5',
           font=('Acumin Variable Concept', 15), justify='center')
l5.place(x=20, y=300)

l6 = Label(LHS, text=f"Processor:{my_system.processor}", bg='#f4f5f5', font=('Acumin Variable Concept', 9),
           justify='center')
l6.place(x=20, y=325)
...
def none():
    ...
    lbl_time.config(text=f'Charging: {str(battery.power_plugged)}')
    lbl_plug.config(text=f'{time}remaining ')
    battery_label = Label(RHS, background='#f4f5f5')
    battery_label.place(x=13, y=50)
    lbl.after(1000, none)
...
lbl = Label(RHS, font=('Acumin Variable Concept,', 40, 'bold'))
lbl.place(x=200, y=40)
lbl_plug = Label(RHS, font=('Acumin Variable Concept,', 10, 'bold'))
lbl_plug.place(x=20, y=100)
lbl_time = Label(RHS, font=('Acumin Variable Concept,', 15, 'bold'))
...
def volume_changed(event):
    ...
    volume1 = cast(interface, POINTER(IAudioEndpointVolume))
    volume1.SetMasterVolumeLevel(-float(get_current_volume_value()), None)
...
def brightness_changed(event):
    pct.get_brightness(get_current_value())


brightness = ttk.Scale(RHS, from_=0, to=60, orient='horizontal', command=brightness_changed, variable=current_value)
...
def weather():
    ...
    def getWeather():
        try:
            ...
            messagebox.showerror('Incorrect City', 'Invalid Entry')
    ...
    label1 = Label(app1, text='HUMIDITY', font=('Helvetica', 15, 'bold'), fg='white', bg='#1ab5ef')
    label1.place(x=250, y=400)

    label1 = Label(app1, text='DESCRIPTION', font=('Helvetica', 15, 'bold'), fg='white', bg='#1ab5ef')
    label1.place(x=430, y=400)

    label1 = Label(app1, text='PRESSURE', font=('Helvetica', 15, 'bold'), fg='white', bg='#1ab5ef')
    label1.place(x=650, y=400)

    # naming the variables
    t = Label(app1, font=('arial', 10))
    t.place(x=400, y=150)

    c = Label(app1, font=('arial', 10, 'bold'), bg='#f4f5f5')
    c.place(x=400, y=250)

    w = Label(app1, font=('arial', 10, 'bold'), bg='#f4f5f5')
    w.place(x=120, y=430)

    h = Label(app1, font=('arial', 10, 'bold'), bg='#f4f5f5')
    h.place(x=280, y=430)
    d = Label(app1, font=('arial', 10, 'bold'), bg='#f4f5f5')
    d.place(x=450, y=430)
    p = Label(app1, font=('arial', 10, 'bold'), bg='#f4f5f5')
    p.place(x=670, y=430)
    app1.mainloop()
...
def game():
    app5 = Toplevel()
    app5.geometry('300x500+1070+100')
    app5.title('Ludo')
...
def screen():
    ...
    my.save(r'C:\Users\KC\Desktop\cc\w.png')
def file():
...
def github():
    wb.register('chrome', None)
    wb.open('https://www.github.com/nduprincekc')
...
RHB = Frame(body, width=470, height=200, bg='#f4f5f5', highlightbackground='#adacb1', highlightthickness=1)
...
app6 = Button(RHB, image=app6_image, bd=0, command=screen)
...
app9 = Button(RHB, image=app9_image, bd=0, command=github)
app9.place(x=270, y=120)

app10_image = PhotoImage(file='Image/App10.png')
app10 = Button(RHB, image=app10_image, bd=0, command=close)
...
# get_current_value()
# brightness_changed(get_current_value())
